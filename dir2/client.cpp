...
Res* get_user_data (CLIENT* cl, Res* (*pfunc) (Params *argp, CLIENT *clnt))
{
	...
	strcpy(params.login, login.data());
	std::cout << "Password: ";
	getline (std::cin, password);
	strcpy(params.password, password.data());

	Res *result = pfunc (&params, cl);
	if (result == NULL) {
...
void message_receiver::check_new_messages (int &cookie, CLIENT *cl)
{
	while (1) 
	{
		...
		receive_res = receive_2 (&params, cl);
		if (receive_res == NULL)
		{
			sleep (5);
			continue;
		}
		if ((receive_res->res.code == FAIL)) 
		{
			std::cout << "FAIL: " << receive_res->res.descr << '\n';
			return;
 		}

		lock.lock();
		for (int i = 0; i < receive_res->data.data_len; ++i) 
		{
			messages.push_back (std::pair <std::string, std::string> (
									std::string (receive_res->data.data_val[i].from),
									std::string (receive_res->data.data_val[i].message)));
		}

		lock.unlock();
		if (receive_res->data.data_len != 0) 
		{
